...
impl ComponentArray {
    ...
    pub(crate) fn push<T>(&mut self, data: T) {
        let size = size_of::<T>();
        let align = align_of::<T>();
    ...
    pub(crate) fn get_cell<T>(&self, index: usize) -> Option<&UnsafeCell<T>> {
        if index >= self.len { return None }
    ...
    pub(crate) fn swap_remove<T>(&mut self, index: usize) -> Option<T> {
        if index >= self.len { return None }
    ...
    pub(crate) fn iter<'a, T>(&'a self) -> ComponentIter<'a, T> {
...
pub struct ComponentIter<'a, T> {
    source: &'a ComponentArray,
    next: usize,
    marker: PhantomData<UnsafeCell<T>>,
}

impl<'a, T> ComponentIter<'a, T> {
    fn new(source: &'a ComponentArray) -> Self {
        Self {
            source,
            next: 0,
            marker: PhantomData,
...
mod test {
    ...
    #[test]
    fn push_and_get() {
        let mut ba = ComponentArray::new::<Obj>(1);
    ...
    #[test]
    fn remove() {
        let mut ba = ComponentArray::new::<Obj>(5);
    ...
    fn zst() {
        const CAP: usize = 2;
        struct Zst;
        let mut ba = ComponentArray::new::<Zst>(CAP);
        for _ in 0..CAP {
            ba.push(Zst);
        }
    ...
    fn speed() {
        ...
        let mut ba = ComponentArray::new::<NewObj>(NUM);
