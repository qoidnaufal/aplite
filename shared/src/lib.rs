...
mod rect {
    ...
    impl<T: GpuPrimitive> Rect<T> {
        ...
        #[inline(always)]
        pub const fn pos(&self) -> Vector2<T> { Vector2::new(self.inner.x(), self.inner.y()) }

        #[inline(always)]
        pub fn set_pos(&mut self, pos: Vector2<T>) { self.inner[0] = pos.x(); self.inner[1] = pos.y() }
        ...
        #[inline(always)]
        pub const fn size(&self) -> Size<T> { Size::new(self.inner.z(), self.inner.w()) }

        #[inline(always)]
        pub fn set_size(&mut self, size: Size<T>) { self.inner[2] = size.width(); self.inner[3] = size.height() }
